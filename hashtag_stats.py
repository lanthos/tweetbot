...
def get_hashtags(tweet):
    ...
    return [tag['text'].lower() for tag in hastags]
...
if __name__ == "__main__":
    ...
    with open(fname, 'r') as f:
        ...
        tweets_with_hashtags_percent = "{:.2f}".format(tweets_with_hashtags / tweets_total * 100)
        tweets_no_hashtags_percent = "{:.2f}".format(tweets_no_hashtags / tweets_total * 100)
        ...
        for tag_count, tweet_count in hashtag_count.items():
            if tag_count > 0:
                percent_total = "{:.2f}".format(tweet_count / tweets_total * 100)
                percent_elite = "{:.2f}".format(tweet_count / tweets_with_hashtags * 100)
                print("{} tweets with {} hashtags ({}% total, {}% elite)".format(tweet_count, tag_count,
